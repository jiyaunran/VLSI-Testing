...
class CIRCUIT
{
    ...
    VALUE NotFunction(VALUE v);
    VALUE NorFunction(GATEPTR gptr);
    VALUE NandFunction(GATEPTR gptr);
    //ass3
    void ParInitPrint(string simulator_name, string benchfile_path);
    string retreive_path_name(string path);
    ofstream simulator;
    void PrintGate(GATEPTR gptr);
    int evaluate_num;

    //ass5
    void GenerateAllBridgeFaultList();
    void BridgeFaultSimVectors();
    void BridgeFaultSim();
    void BridgeFaultSimEvaluate(GATEPTR gptr);
    bool CheckBridgeFaultyGate(Bridge_FAULT* fptr);

    list <Bridge_FAULT*> BridgeFlist;
    list <Bridge_FAULT*> BridgeUFlist;
};
#endif
