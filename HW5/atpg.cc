...
const bool checkpoint_ver=false;
...
void CIRCUIT::GenerateAllFaultList()
{
    ...
    if(checkpoint_ver){
        for (unsigned int i=0; i < No_Gate(); i++){
            if (Netlist[i]->No_Fanout()>1){
                // branch
                gptr = Netlist[i];
                for(j=0; j<gptr->No_Fanout();j++){
                    fanout = gptr->Fanout(j);
                    //if (j==0) cout << gptr->GetName() << ' ' << fanout->GetName() << endl;
...
//ass5
void CIRCUIT::GenerateAllBridgeFaultList()
{
    cout << "Generate bridging fault list" << endl;
    register unsigned i, j;
    GATEFUNC fun;
    GATEPTR gptr1, gptr2, fanout;
    Bridge_FAULT *fptr;
    
    for(int i=0; i<GetMaxLevel(); i++){
        int prev_net=-1;
        //cout << "level: " << i << endl;
        for(int j=0; j<No_Gate(); j++){
            if(Gate(j)->GetLevel() == i){
                if(prev_net != -1){
                    gptr1 = Gate(prev_net);
                    gptr2 = Gate(j);

                    fun = gptr1->GetFunction();
                    
                    if (fun != G_PO) {//skip PO
                        //add stem 1000 fault to Flist
                        fptr = new Bridge_FAULT(gptr1, gptr2, gptr1, gptr2, S0);
                        BridgeFlist.push_front(fptr);
                        //add stem 0111 fault to Flist
                        fptr = new Bridge_FAULT(gptr1, gptr2, gptr1, gptr2, S1);
                        BridgeFlist.push_front(fptr);
                    }

                    //add branch fault
                    // if (gptr1->No_Fanout() != 1) {//skip no branch faults
                    //     for (j = 0;j< gptr1->No_Fanout();++j) {
                    //         cout << j << endl;
                    //         fanout = gptr1->Fanout(j);
                    //         fptr = new Bridge_FAULT(gptr1, gptr2, fanout, S0);
                    //         fptr->SetBranch(true);
                    //         BridgeFlist.push_front(fptr);
                    //         fptr = new Bridge_FAULT(gptr1, gptr2, fanout, S1);
                    //         fptr->SetBranch(true);
                    //         BridgeFlist.push_front(fptr);
                    //     } 
                    // }

                    // fun = gptr2->GetFunction();
                    // if (fun == G_PO) { continue; }
                    // //add stem 0100 fault to Flist
                    // fptr = new Bridge_FAULT(gptr1, gptr2, gptr2, S0);
                    // BridgeFlist.push_front(fptr);
                    // //add stem 1011 fault to Flist
                    // fptr = new Bridge_FAULT(gptr1, gptr2, gptr2, S1);
                    // BridgeFlist.push_front(fptr);

                    // if (gptr1->No_Fanout() == 1) { continue; }
                    
                    // for (j = 0;j< gptr2->No_Fanout();++j) {
                    //     fanout = gptr2->Fanout(j);
                    //     fptr = new Bridge_FAULT(gptr1, gptr2, fanout, S0);
                    //     fptr->SetBranch(true);
                    //     BridgeFlist.push_front(fptr);
                    //     fptr = new Bridge_FAULT(gptr1, gptr2, fanout, S1);
    ...
    // for (i = 1;i<No_Gate();++i) {
    //     gptr1 = Netlist[i-1]; gptr2 = Netlist[i]; fun = gptr1->GetFunction();
    //     if (fun != G_PO) {//skip PO
    //         //add stem 1000 fault to Flist
    //         fptr = new Bridge_FAULT(gptr1, gptr2, gptr1, S0);
    //         BridgeFlist.push_front(fptr);
    //         //add stem 0111 fault to Flist
    //         fptr = new Bridge_FAULT(gptr1, gptr2, gptr1, S1);
    //         BridgeFlist.push_front(fptr);
    //     }
